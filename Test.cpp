...
void testEmptySLLSetSize(){
	SLLSet obj = SLLSet();
	int expected = 0;
	ASSERT_EQUAL(expected, obj.getSize());

}

void testEmptySLLSetCopy(){
	SLLSet obj = SLLSet();
	SLLSet expected = SLLSet();
	ASSERT_EQUAL(expected.toString(),obj.copy().toString());

}

void testMyBool(){
	int inputArray[7] = {1,7,4,8,10};
	SLLSet obj = SLLSet(inputArray, 7);
	ASSERT_EQUAL(true, obj.isIn(1));
	ASSERT_EQUAL(true, obj.isIn(3));
	ASSERT_EQUAL(false, obj.isIn(3));
	ASSERT_EQUAL(false, obj.isIn(0));
}
...
void testEmptyRemove(){
	...
	obj.remove(7);
	obj.remove(8);
	ASSERT_EQUAL(SLLSet().toString(),obj.toString());

}

void testEmptyUnion(){
	SLLSet obj1 = SLLSet(), obj2 = SLLSet();
	ASSERT_EQUAL(SLLSet().toString(), obj2.setUnion(obj1).toString());

}
void testEmptyIntersection(){
	SLLSet obj1 = SLLSet(), obj2 = SLLSet();
	ASSERT_EQUAL(SLLSet().toString(), obj2.intersection(obj1).toString());

}
void testEmptyDifference(){
	SLLSet obj1 = SLLSet(), obj2 = SLLSet();
	ASSERT_EQUAL(SLLSet().toString(), obj2.difference(obj1).toString());

}

void testEmptyString() {
	SLLSet obj = SLLSet();
	ASSERT_EQUAL("", obj.toString());
}
...
bool runAllTests(int argc, char const *argv[]) {
	...
	s.push_back(CUTE(testEmptySLLSetSize));
	s.push_back(CUTE(testEmptySLLSetCopy));
	s.push_back(CUTE(testMyBool));
	s.push_back(CUTE(testEmptyRemove));
	s.push_back(CUTE(testEmptyUnion));
	s.push_back(CUTE(testEmptyIntersection));
	s.push_back(CUTE(testEmptyDifference));
